 ...
 * a board-provided sense resistor and integrates it in hardware into a charge
 * accumulation register (CAR). This driver runs the same battery-meter
 * algorithm MediaTek's downstream kernels run on top of that hardware
 * (drivers/power/mt81xx, CONFIG_SOC_BY_HW_FG), so that a board ported from
 * such a kernel reports the same state of charge it did before:
 *
 *  - At boot the state of charge is seeded from the open-circuit voltage the
 *    PMIC latched by itself at power-on, cross-checked against the live
 *    voltage compensated for I*R, and against the value the previous boot
 *    left in an RTC spare register.
 *  - From then on it is coulomb counted: DOD1 = DOD0 - CAR / Qmax, where
 *    Qmax is interpolated from per-temperature capacities.
 *  - The OCV, internal-resistance and Qmax tables are all per temperature and
 *    re-interpolated as the pack temperature (from an NTC on the PMIC's
 *    auxiliary ADC) moves.
 *  - A charger reporting full re-anchors the counter at 100 %, and the
 *    displayed percentage moves towards the computed one by at most one point
 *    per step, so it never jumps.
 *
 * Every table and constant is taken from the vendor's own tuning for the
 * battery fitted; nothing here is measured or estimated by this driver.
 *
...
/*
 * The auxiliary ADC latches a conversion of the battery channel when the PMIC
 * powers on, before any load is applied: the "hardware OCV". 10 bits over
 * 1.2 V behind the same 1:4 divider as the live channel, plus the vendor's
 * fixed per-chip correction.
 */
...
/*
 * The vendor keeps the last state of charge in the RTC's alarm-hour register,
 * bits 8..14, which the alarm never uses; it survives a reboot and is how a
 * restart avoids a jump. The RTC needs a write-trigger after any register
 * write, as rtc-mt6397 does.
 */
#define MT6397_RTC_BASE			0xe000
#define MT6397_RTC_BBPU			(MT6397_RTC_BASE + 0x0000)
#define MT6397_RTC_BBPU_CBUSY		BIT(6)
#define MT6397_RTC_AL_HOU		(MT6397_RTC_BASE + 0x001c)
#define MT6397_RTC_AL_HOU_SOC		GENMASK(14, 8)
#define MT6397_RTC_WRTGR		(MT6397_RTC_BASE + 0x003c)
#define MT6397_RTC_TIMEOUT_US		100000
...
#define MT6397_BAT_MAX_PROFILES		5
...
struct mt6397_battery {
	...
	u32 car_tune;
	bool ocv2cv;
	int step_of_qmax_mah;
	int cv_current_01ma;
	int poweron_low_cap_tol;
	u32 ntc_pullup_ohm;
	u32 ntc_pullup_mv;
	u32 ntc_pulldown_ohm;
	struct mt6397_bat_ntc_point *ntc_tbl;
	int nntc;
	struct mt6397_bat_profile prof[MT6397_BAT_MAX_PROFILES];
	int nprof;
	int npts;
...
static int mt6397_battery_fg_enable(struct mt6397_battery *bat)
{
	...
	return mt6397_battery_reset_car(bat);
}
...
static int mt6397_battery_rtc_get_soc(struct mt6397_battery *bat)
{
	...
	if (regmap_read(bat->regmap, MT6397_RTC_AL_HOU, &reg))
		return 0;

	return FIELD_GET(MT6397_RTC_AL_HOU_SOC, reg);
}
...
static void mt6397_battery_rtc_set_soc(struct mt6397_battery *bat, int soc)
{
	...
	ret = regmap_read(bat->regmap, MT6397_RTC_AL_HOU, &reg);
	if (ret || FIELD_GET(MT6397_RTC_AL_HOU_SOC, reg) == soc)
		return;

	/*
	 * rtc-mt6397 masks the alarm hour to its low 5 bits on write, so this
	 * field survives alarms being set; the two drivers can still interleave
	 * a write and a trigger, which is harmless because a trigger commits
	 * whatever is pending and both sides wait for it.
	 */
	ret = regmap_update_bits(bat->regmap, MT6397_RTC_AL_HOU,
				 MT6397_RTC_AL_HOU_SOC,
				 FIELD_PREP(MT6397_RTC_AL_HOU_SOC, soc));
	if (ret)
		return;

	ret = regmap_write(bat->regmap, MT6397_RTC_WRTGR, 1);
	if (ret)
		return;

	regmap_read_poll_timeout(bat->regmap, MT6397_RTC_BBPU, reg,
				 !(reg & MT6397_RTC_BBPU_CBUSY),
				 100, MT6397_RTC_TIMEOUT_US);
}
...
/*
 * The profiles are ordered by falling voltage. Given a voltage, interpolate
 * the other column; beyond either end return that end's value, which for the
 * OCV tables is 0 and 100 % depth of discharge, and for the resistance tables
 * the end resistance, exactly as the vendor's lookups do.
 */
...
static int mt6397_bat_qmax_by_current(struct mt6397_battery *bat, int i_01ma)
{
	...
	for (idx = 0; idx < bat->npts - 1; idx++) {
		...
		if (bat->ocv_t[idx].mv - vdrop < MT6397_BAT_SYSTEM_OFF_MV) {
			qmax = idx <= 1 ? bat->step_of_qmax_mah :
					  (idx - 1) * bat->step_of_qmax_mah;
			break;
		}
	}
...
/*
 * I*R in mV for the present current at a given terminal voltage, iterating
 * because the resistance itself is looked up by voltage. Negative while
 * charging, so that voltage + compensation is the open-circuit voltage
 * either way.
 */
...
static int mt6397_bat_read_temp(struct mt6397_battery *bat, int *temp_c)
{
	s64 r, r_tmp, crit;
	int mv, i, ret;
	...
	ret = iio_read_channel_processed(bat->ntc, &mv);
	if (ret < 0)
		return ret;

	/*
	 * The NTC's ground return is the sense resistor's far side, so the
	 * voltage across it shifts the reading by I*R_sense.
	 */
	if (bat->charging)
		mv -= bat->cur_01ma / 10 * (int)bat->sense_mohm / 1000;
	else
		mv += bat->cur_01ma / 10 * (int)bat->sense_mohm / 1000;

	crit = (s64)bat->ntc_tbl[0].ohm * bat->ntc_pullup_mv;
	crit = div_s64(crit, bat->ntc_tbl[0].ohm + bat->ntc_pullup_mv);
	if (mv >= crit) {
		r_tmp = bat->ntc_tbl[0].ohm;
	} else {
		r_tmp = (s64)bat->ntc_pullup_ohm * mv;
		r_tmp = div_s64(r_tmp, (int)bat->ntc_pullup_mv - mv);
	}
	r = r_tmp * bat->ntc_pulldown_ohm;
	r = div_s64(r, abs((s64)bat->ntc_pulldown_ohm - r_tmp));

	if (r >= bat->ntc_tbl[0].ohm) {
		*temp_c = bat->ntc_tbl[0].temp_c;
	} else if (r <= bat->ntc_tbl[bat->nntc - 1].ohm) {
		*temp_c = bat->ntc_tbl[bat->nntc - 1].temp_c;
	} else {
		for (i = 1; i < bat->nntc; i++) {
			if (r >= bat->ntc_tbl[i].ohm) {
				s64 r1 = bat->ntc_tbl[i - 1].ohm, r2 = bat->ntc_tbl[i].ohm;
				int t1 = bat->ntc_tbl[i - 1].temp_c, t2 = bat->ntc_tbl[i].temp_c;

				*temp_c = div_s64((r - r2) * t1 + (r1 - r) * t2,
						  (s32)(r1 - r2));
				break;
			}
		}
...
static void mt6397_bat_update_temp(struct mt6397_battery *bat)
{
	...
	if (mt6397_bat_read_temp(bat, &temp_c))
		temp_c = bat->temp_c;
	bat->temp_c = temp_c;
...
static void mt6397_bat_read_charger(struct mt6397_battery *bat)
{
	...
	/* A pack that already reads 100 % when the cable goes in is full. */
	if (bat->ui_soc == 100)
		bat->full = true;
...
/*
 * The displayed percentage. It only ever moves one point at a time towards
 * the gauge's value, sits at 99 % until the charger says full, ramps to 100 %
 * once it does and re-anchors the counter there, and never shows 0 % while
 * the pack is above the system-off voltage.
 */
static void mt6397_bat_update_ui(struct mt6397_battery *bat)
{
	...
	mt6397_battery_rtc_set_soc(bat, max(bat->cap_by_c, 1));
}
...
/*
 * Boot: seed the depth of discharge. Three candidates, in the vendor's order
 * of trust -- the hardware-latched power-on OCV, the live voltage compensated
 * for load (which wins if the two disagree by more than 5 %), and what the
 * previous boot left in the RTC (which wins if it is within tolerance of
 * either, so a reboot does not jump).
 */
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:
		if (bat->info && bat->info->charge_full_design_uah > 0)
			val->intval = bat->info->charge_full_design_uah;
		else
			val->intval = mt6397_bat_qmax(bat, 25, false) * 1000;
		break;
	case POWER_SUPPLY_PROP_CHARGE_NOW:
		val->intval = bat->qmax_aging_mah * 1000 / 100 * bat->cap_by_c;
		break;
	case POWER_SUPPLY_PROP_TEMP:
		val->intval = bat->temp_c * 10;
		break;
...
static void mt6397_battery_external_power_changed(struct power_supply *psy)
{
	...
	mod_delayed_work(system_wq, &bat->work, HZ);
}
...
static int mt6397_bat_read_pairs(struct device *dev, const char *name,
				 s32 **out, int *count)
{
	int n, ret;
	u32 *buf;
	...
	if (n < 0)
		return n;
	if (n < 4 || n & 1)
		return dev_err_probe(dev, -EINVAL, "%s: needs value pairs\n", name);

	buf = devm_kcalloc(dev, n, sizeof(*buf), GFP_KERNEL);
	if (!buf)
		return -ENOMEM;
...
static int mt6397_bat_parse_dt(struct mt6397_battery *bat)
{
	...
	u32 temps[MT6397_BAT_MAX_PROFILES], qmax[MT6397_BAT_MAX_PROFILES];
	u32 qmax_hc[MT6397_BAT_MAX_PROFILES], v;
	char name[40];
	s32 *pairs;
	int n, i, ret;
	...
	v = 54;
	device_property_read_u32(dev, "mediatek,step-of-qmax-milliamp-hours", &v);
	bat->step_of_qmax_mah = v;
	...
	/* the NTC */
	if (!device_property_read_u32(dev, "mediatek,ntc-pull-up-ohms",
				      &bat->ntc_pullup_ohm)) {
		bat->ntc_pullup_mv = 1200;
		device_property_read_u32(dev, "mediatek,ntc-pull-up-millivolt",
					 &bat->ntc_pullup_mv);
		bat->ntc_pulldown_ohm = 100000000;
		device_property_read_u32(dev, "mediatek,ntc-pull-down-ohms",
					 &bat->ntc_pulldown_ohm);

		ret = mt6397_bat_read_pairs(dev, "mediatek,ntc-table", &pairs, &n);
		if (ret)
			return ret;
		bat->ntc_tbl = (struct mt6397_bat_ntc_point *)pairs;
		bat->nntc = n;
	}

	/* the per-temperature profiles */
	n = device_property_count_u32(dev, "mediatek,profile-temperatures");
	if (n < 1 || n > MT6397_BAT_MAX_PROFILES)
		return dev_err_probe(dev, -EINVAL,
				     "mediatek,profile-temperatures: 1..%d entries\n",
				     MT6397_BAT_MAX_PROFILES);
	bat->nprof = n;

	ret = device_property_read_u32_array(dev, "mediatek,profile-temperatures",
					     temps, n);
	if (ret)
		return ret;
	ret = device_property_read_u32_array(dev, "mediatek,qmax-milliamp-hours",
					     qmax, n);
	if (ret)
		return dev_err_probe(dev, ret, "mediatek,qmax-milliamp-hours\n");
	ret = device_property_read_u32_array(dev,
					     "mediatek,qmax-high-current-milliamp-hours",
					     qmax_hc, n);
	if (ret)
		memcpy(qmax_hc, qmax, sizeof(qmax));

	for (i = 0; i < n; i++) {
		int npts;

		bat->prof[i].temp_c = (s32)temps[i];
		bat->prof[i].qmax_mah = qmax[i];
		bat->prof[i].qmax_hc_mah = qmax_hc[i];

		if (i && bat->prof[i].temp_c <= bat->prof[i - 1].temp_c)
			return dev_err_probe(dev, -EINVAL,
					     "profile temperatures must ascend\n");

		snprintf(name, sizeof(name), "mediatek,ocv-profile-%d", i);
		ret = mt6397_bat_read_pairs(dev, name, &pairs, &npts);
		if (ret)
			return dev_err_probe(dev, ret, "%s\n", name);
		bat->prof[i].ocv = (struct mt6397_bat_point *)pairs;
		if (!i)
			bat->npts = npts;
		else if (npts != bat->npts)
			return dev_err_probe(dev, -EINVAL,
					     "%s: %d points, expected %d\n",
					     name, npts, bat->npts);

		snprintf(name, sizeof(name), "mediatek,resistance-profile-%d", i);
		ret = mt6397_bat_read_pairs(dev, name, &pairs, &npts);
		if (ret)
			return dev_err_probe(dev, ret, "%s\n", name);
		bat->prof[i].res = (struct mt6397_bat_point *)pairs;
		if (npts != bat->npts)
			return dev_err_probe(dev, -EINVAL,
					     "%s: %d points, expected %d\n",
					     name, npts, bat->npts);
...
static int mt6397_battery_probe(struct platform_device *pdev)
{
	...
	if (bat->ntc_tbl) {
		bat->ntc = devm_iio_channel_get(dev, "temperature");
		if (IS_ERR(bat->ntc))
			return dev_err_probe(dev, PTR_ERR(bat->ntc),
					     "failed to get the temperature channel\n");
	}
	...
	if (power_supply_get_battery_info(bat->psy, &bat->info))
		bat->info = NULL;
...
static int mt6397_battery_resume(struct device *dev)
{
	...
	/*
	 * The accumulator counted through the sleep; the vendor lets the
	 * display drop straight away after a resume rather than waiting out
	 * the 30 s sync period.
	 */
	scoped_guard(mutex, &bat->lock)
		bat->refresh_ui = true;
	schedule_delayed_work(&bat->work, 0);
